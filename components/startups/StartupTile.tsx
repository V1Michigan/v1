    ...
    <>
      ...
      <li onClick={() => setDialogOpen(true)} className="m-0 p-0 list-none rounded-md">
              ...
              <div>
                ...
                <div className="flex gap-1">
                  {industries.map((industry) => (
                    <span className="">
                      <p className="text-sm my-1 w-fit px-2 bg-slate-300 rounded-xl">{industry}</p>
                    </span>
                  ))}
                </div>
              </div>
        ...
        <div className="flex flex-col gap-2 mt-2">
          ...
          <h1 className="text-black font-md font-figtree font-semibold">{name}</h1>
          <button
            type="button"
                ...
                <Dialog.Panel className="w-full max-w-xl transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                  <div className="relative flex flex-col gap-y-4">
                    <div className="flex gap-x-8">
                      <img src={logo} className="w-48 rounded-lg" alt={`${name} logo`} />
